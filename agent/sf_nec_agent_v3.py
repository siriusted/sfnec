...
class SFNECAgentV3:
    '''
    This is the agent for trying out SF & GPI integration with NEC in a new way.

    The idea is to allow for each new phase, the agent to learn without GPI for half the phase,
    then use GPI for the second half.

    We think this will allow for better complementary benefit by using speed of learning on first
    half enable by NEC, before trying other old policies
    '''
    ...
    def new_episode(self, obs, info):
        if self.training:
            # trackers for computing N-step returns and updating replay and dnd memories
            self.observations, self.keys, self.actions, self.phis, self.max_psis = [], [], [], [], []
            self.episodes += 1
    ...
    def add_reward_function(self, reward_function, reward_func_descr=None, w=None):
        ...
        self.replay_buffer = eu.misc.call_function_from_config(
            ...
            (self.config.phi_size, ), # shape of returns
            func_attribute_name='cls'
        )
    ...
    def calc_own_max_action(self, psi_values):
        '''Calculates max action using current network - used to learn specialized policy'''

        q_values = psi_values @ self.w_per_reward_func[self.active_reward_func_idx]
        _, max_action = utils.select_max_value(q_values)
        return max_action

    def is_beyond_midpoint_for_task(self):
        '''
        Check if agent has gotten to middle of number of allowed transitions on a task

        We use this to know when to switch from own action selection to gpi action selection
        '''

        return self.steps_in_active_task > (self.config.n_max_steps / 2)

    def dogpi(self):
        return self.config.action_selection == 'gpi' and self.is_beyond_midpoint_for_task()

    def step(self, obs, info = {}):
        self.steps += 1
        self.steps_in_active_task += 1
        self.gpi_mode_active = self.dogpi()
        ...
        # we add here a condition to do gpi only from second half of a phase
        if self.gpi_mode_active:
            # do gpi procedure
            gpi_action, self.gpi_optimal_policy_idx = self.calc_gpi_action(obs)
            bootstrap_action = gpi_action if self.config.bootstrap_psi_mode == 'gpi' else self.calc_own_max_action(psi_values)
            max_psi_value = psi_values[bootstrap_action]
            
        else:
            # nogpi == calc max action for this policy only 
            own_greedy_action = self.calc_own_max_action(psi_values)
            max_psi_value = psi_values[own_greedy_action]
        
        # epsilon-greedy
        if np.random.rand() < eps:
            action = np.random.choice(np.arange(self.action_space.n))
        elif self.gpi_mode_active:
            action = gpi_action
        else:
        ...
        if self.training:
            ...
            self.max_psis.append(max_psi_value)
    ...
    def update(self, transition):
        ...
        # update the reward weights if learning them
        if self.learn_w:
            w = self.w_per_reward_func[self.active_reward_func_idx]
            w_loss = reward - (phi @ w)
            # monitor loss of w
            log.add_value(f'w_loss_phase_{self.active_reward_func_idx}', w_loss)
        ...
        if self.gpi_mode_active and self.config.update_gpi_optimal_policy:
            self.update_gpi_optimal_policy(obs, action, phi, next_obs, done)
        ...
        if done:
            ...
            for t in range(episode_length - 1, -1, -1):
                ...
                if episode_length - t > self.config.n_step_horizon:
                    n_step_returns[t] = returns[t] + self.config.gamma ** self.config.n_step_horizon * (self.max_psis[t + self.config.n_step_horizon] - returns[t + self.config.n_step_horizon])
    ...
    def _update_after_horizon(self, done):
        ...
        if isEpisodeBeyondHorizon:
            ...
            n_step_return = np.asarray(sum([self.config.gamma ** i * self.phis[episode_length - (self.config.n_step_horizon - i + 1)] for i in range(self.config.n_step_horizon)]) \
                            + self.config.gamma ** self.config.n_step_horizon * self.max_psis[episode_length - 1], dtype=np.float32)
    ...
    def update_gpi_optimal_policy(self, obs, action, phi, next_obs, done):
        if self.gpi_optimal_policy_idx != self.active_reward_func_idx:
            ...
            if not done:
                ...
                target = torch.from_numpy(np.float32(phi)).to(self.config.device)
    ...
    def calc_gpi_action(self, obs):
        ...
        return max_action, max_policy_idx
    ...
    def get_q_values(self, observations, actions=None):
        """
        Computes q_values for observation, action pairs passed in.

        Used for testing
        """
        psis = self.get_psi_values(observations, actions)

        return psis @ self.w_per_reward_func[self.active_reward_func_idx]

    def calc_psi_function(self, obs, action=None, reward_function_idx=None):
    ...
    def prepare_for_storage(self):
        ...
        agent.observations = agent.keys = agent.actions = agent.phis = agent.max_psis = None
        agent.loss_func = agent.replay_buffer = agent.optimizer = agent.gpi_optimal_policy_optimizer = None
        agent.episodes_per_reward_func_counter = agent.obj_visits_per_reward_func_counter = None
        return agent