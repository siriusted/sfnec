...
class ExperienceReplay:
    def __init__(self, observation_shape, capacity = 100000, returns_shape = ()):
        self.capacity = capacity
        self.actions = np.empty((capacity, ), dtype = np.int64)
        self.observations = np.empty((capacity, ) + observation_shape, dtype = np.float32)
        self.returns = np.empty((capacity, ) + returns_shape, dtype = np.float32) # test that previous experiments still work
        self.idx = 0
